...
* `snake_case` – стиль именования переменных и функций
...
***Подробнее про правила написания кода написано в официальном постановлении `PEP8`:*** https://peps.python.org/pep-0008/
...
Используются они, в основном, в условных конструкциях, которые мы расcмотрим позже
...
`or` – логическое `ИЛИ`, то есть хотя б одно условие в конструкции `(условие 1)||(условие 2)` должно быть `True`
...
При обращении к несуществующему индексу в Python будет выдаваться ошибка.
...
```py
dic = {
    '''название индекса 1''': '''значение 1''',
    '''название индекса 2''': '''значение 2''',
    ...
    '''название индекса N''': '''значение N'''}
```
